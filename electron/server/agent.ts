...
function buildBrowserSurfacePrompt(surface: ConversationSurface) {
  const common = [
    ...
    "- action=\"script\" returns compact page metadata by default and omits full history, elements, and page text from the tool payload. Call action=\"snapshot\" next, or set includeState=true only when the script response itself needs the full page state.",
